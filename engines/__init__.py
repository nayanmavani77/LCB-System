...
to any other file:
  ...
  - all shared CLI flags (--rr, --sl-*, --tf, --max-spread, --order-age,
    --flow-lo/hi, --set KEY=VALUE) and per-symbol defaults from
    core/symbols.py
...
Simplest path: subclass LRevStrategy and override what differs. Optional
class attribute CLI_DEFAULTS = {...} declares config overrides the runners
apply as the engine's base (explicit CLI flags still win). Optional
describe(cfg) / status() methods customise the config line and heartbeat.
"""
